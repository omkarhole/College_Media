...
router.put('/profile', verifyToken, validateProfileUpdate, checkValidation, async (req, res, next) => {
  try {
    ...
    if (dbConnection && dbConnection.useMongoDB) {
      // Use MongoDB
      const oldUser = await UserMongo.findById(req.userId).select('firstName lastName bio');
      ...
      if (oldUser && updatedUser) {
        AuditService.log({
          ...
          oldValue: { firstName: oldUser.firstName, lastName: oldUser.lastName, bio: oldUser.bio },
          newValue: { firstName: updatedUser.firstName, lastName: updatedUser.lastName, bio: updatedUser.bio },
          req
        });
      }
